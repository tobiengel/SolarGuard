...
/*---------------------------------------------------------------------------*/
static void
pub_handler(const char *topic, uint16_t topic_len, uint8_t *chunk,
            uint16_t chunk_len)
{
...
{
  int len = snprintf(pub_topic, BUFFER_SIZE, "fhem/%s/panelcontrol", ROOM);

  /* len < 0: Error. Len >= BUFFER_SIZE: Buffer too small */
  LOG_INFO("Pub Topic: %s, Buffer %d\n", pub_topic, BUFFER_SIZE);
...
construct_sub_topic(void)
{
  ...
  //                   conf.cmd_type);
  int len = snprintf(sub_topic, BUFFER_SIZE, "fhem/%s/audiodisplay", ROOM);
  LOG_INFO("Sub Topic: %s, Buffer %d\n", sub_topic, BUFFER_SIZE);
  /* len < 0: Error. Len >= BUFFER_SIZE: Buffer too small */
  if(len < 0 || len >= BUFFER_SIZE) {
...
/*---------------------------------------------------------------------------*/
/*static void
publishStr(char* topic, char* msg)
{
      ...
      int len = snprintf(buf_ptr, remaining, msg);
      ...
      len = snprintf(pub_topic, BUFFER_SIZE, topic);
...
PROCESS_THREAD(mqtt_client_process, ev, data)
{
  ...
  etimer_set(&cyclic_send_timer, (CLOCK_SECOND * 5));
